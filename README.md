...
Releases are automatic. Every push to a release branch is validated, and if the commits since the last release warrant one, a version is published without any manual trigger — so merging a pull request is a release decision. Versions are calculated from [Conventional Commits](https://www.conventionalcommits.org/), and every accepted type releases something, so a docs-only or dependency-only merge still publishes a patch version.

Publication is driven by the resulting GitHub release and packages the release tag rather than the branch. A published version is never overwritten, and re-running the publish workflow completes an interrupted publish without moving the tag.
...
`main` is the default branch and the stable channel. `next` is the release-candidate channel: it exists whenever a change requires validation in a customer-facing environment before it is promoted to stable. Once that version is promoted into `main`, `next` is deleted, and it is recreated from `main` when a future prerelease channel is needed. Only these two branches publish; pull request builds validate but never publish.

A change that belongs in both channels goes into `main` first and is forward-ported to `next` afterwards. This keeps the stable channel complete by construction: a fix can never exist only on the prerelease channel. Merge `main` into `next` after a stable release rather than letting the branches drift; that merge publishes a release candidate containing the fix. A change that applies to only one channel stays on that channel.

#### Generated release commits
